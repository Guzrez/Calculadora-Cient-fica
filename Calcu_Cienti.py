...
notebook.add(frame_funcion, text="Función asignada")
...
def reiniciar():
    ...
    combobox_figura.config(state="readonly")
    entry1.delete(0, END)
    entry2.delete(0, END)
...
# -------------------------------------Espacio para el diseño del widget que les toco en el frame_funcion---------------------------------------------------------------------


def mostrar_mensaje(tipo):
    if tipo == "Info":
        respuesta = messagebox.askokcancel("Información", "Este es un mensaje de información. ¿Aceptar o cancelar?")
        if respuesta:
            label_resultado.config(text="Botón presionado: Aceptar")
        else:
            label_resultado.config(text="Botón presionado: Cancelar")
    
    elif tipo == "Advertencia":
        respuesta = messagebox.askretrycancel("Advertencia", "Este es un mensaje de advertencia. ¿Reintentar o cancelar?")
        if respuesta:
            label_resultado.config(text="Botón presionado: Reintentar")
        else:
            label_resultado.config(text="Botón presionado: Cancelar")
    
    elif tipo == "Error":
        respuesta = messagebox.showerror("Error", "Imposible realizar esta acción, eres demasiado guapo.")
        if respuesta:
          label_resultado.config(text="Botón presionado: Aceptar")
            
    
    elif tipo == "Pregunta":
        respuesta = messagebox.askquestion("Pregunta", "¿Deseas salir?")
        if respuesta == 'yes':
            window.quit()  # Cierra la ventana principal
        else:
            label_resultado.config(text="Botón presionado: No")

# Función para reiniciar la selección de radiobuttons
def repetir_seleccion():
    tipo_mensaje.set(None)
    label_resultado.config(text="")
    boton_repetir.pack_forget()

# Crear los Radiobuttons para seleccionar el tipo de Messagebox
label_tipo = ttk.Label(frame_funcion, text="Seleccione el tipo de mensaje:")
label_tipo.pack(pady=10)


tipo_mensaje = tk.StringVar()

radiobutton_info = ttk.Radiobutton(frame_funcion, text="Información", variable=tipo_mensaje, value="Info",
                                   command=lambda: mostrar_mensaje(tipo_mensaje.get()))
radiobutton_advertencia = ttk.Radiobutton(frame_funcion, text="Advertencia", variable=tipo_mensaje, value="Advertencia",
                                          command=lambda: mostrar_mensaje(tipo_mensaje.get()))
radiobutton_error = ttk.Radiobutton(frame_funcion, text="Error", variable=tipo_mensaje, value="Error",
                                    command=lambda: mostrar_mensaje(tipo_mensaje.get()))
radiobutton_pregunta = ttk.Radiobutton(frame_funcion, text="Pregunta", variable=tipo_mensaje, value="Pregunta",
                                       
                                       command=lambda: mostrar_mensaje(tipo_mensaje.get()))

# Colocar los Radiobuttons en el frame
radiobutton_info.pack(anchor=tk.W, padx=20)
radiobutton_advertencia.pack(anchor=tk.W, padx=20)
radiobutton_error.pack(anchor=tk.W, padx=20)
radiobutton_pregunta.pack(anchor=tk.W, padx=20)

# Etiqueta para mostrar qué botón fue presionado
label_resultado = ttk.Label(frame_funcion, text="", font=("Arial", 12))
label_resultado.pack(pady=10)

# Botón de repetir que se muestra después de seleccionar un messagebox
boton_repetir = ttk.Button(frame_funcion, text="Repetir",
                           
                           command=repetir_seleccion)
...
ruta_imagen = r"C:\Users\omarp\Downloads\Imagen de WhatsApp 2024-10-11 a las 13.02.56_dd495279.jpg" #Cambia esta ruta si es necesario
